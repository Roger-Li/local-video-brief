...
- Run backend tests with `python3 -m pytest backend/tests` (68 tests).
...
- `extract_json()` uses `json.JSONDecoder().raw_decode()` to handle trailing characters, and strips `<think>` blocks as a safety net.
...
- **Lecture study pack (Phase B+C)**: Optional `study_pack` in result payload behind `OVS_ENABLE_STUDY_PACK` flag, with frontend study guide tab and Markdown export. See `artifacts/plan/LECTURE_PLAN_V2.md` and `artifacts/plan/LECTURE_PLAN_V2_IMPLEMENTATION.md` for full design.
